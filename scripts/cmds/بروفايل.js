module.exports = {
	config: {
		name: "تنظيف", // Name of command, it must be unique to identify with other commands
	...
	onStart: async function ({ api, args, message, event, threadsData, usersData, dashBoardData, globalData, threadModel, userModel, dashBoardModel, globalModel, role, commandName, getLang }) {
		...
		const allThreadData = threadsData.getAll();

for (const thread of allThreadData) {

 const membersList = thread.members.filter((member) => member.userID);

 await threadsData.set(thread.threadID, { members: membersList });

}

message.reply("تم تنظيف البيانات يا مطور");
