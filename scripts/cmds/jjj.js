...
if(!inpp || inpp.length < 2 || isNaN(Number(inpp[0]))) return (message.reply("Use correct Syntax"))
  ...
  if(!itm) return message.reply("هذا المعرف غير موجود")
let inp = inpp[1]
  let strm = []
  let clr = ""
  if(inpp[2]){clr = inpp[2]} else{clr =itm.clr}
  try{
let idd = 1
    for(var e of jsn){
...
ctx.strokeStyle = "white"
     ctx.lineWidth = 1.5
// ctx.strokeText(inp, cnv.width/2, 308)
     ctx.strokeText(inp, cnv.width/2, 432)
     ctx.strokeText(inp, cnv.width/2, 708)
    // ctx.strokeText(inp, cnv.width/2, 840)
  ...
  await fs.writeFileSync(__dirname + `/ttmp/asd${idd}.png`, imgBuffer)
      strm.push(fs.createReadStream(__dirname + `/ttmp/asd${idd}.png`))
idd++
                                    }
 // fs.writeFileSync('asd.png', imgBuffer)
message.reply({body:"🎇🎇",attachment:strm})
